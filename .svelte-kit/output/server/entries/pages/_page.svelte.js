...
function _page($$renderer, $$props) {
  $$renderer.component(($$renderer2) => {
    const cards = [
      { title: "Game Score Trackers", href: "/games" },
      { title: "Workout Tracker", href: "/workout" },
      { title: "Ren's Tools", href: "/ren" }
    ];
    ...
    for (let $$index = 0, $$length = each_array.length; $$index < $$length; $$index++) {
      ...
      $$renderer2.push(`<a class="card svelte-1uha8ag"${attr("href", `${stringify(base)}${stringify(card.href)}`)}><h3 class="svelte-1uha8ag">${escape_html(card.title)}</h3> <span class="card-arrow svelte-1uha8ag" aria-hidden="true">→</span></a>`);
    }
